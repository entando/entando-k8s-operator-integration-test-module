...
abstract class BareBonesDeployableSpecBase extends Specification implements TestFixtureFactory, PodBehavior {
    ...
    def "the secret is built and is mounted in the standard secret mount root in the container"() {

        given: "I have created an EntandoCustomResource"
        ...
        and: "I use the BareBonesDeployable with the BareBonesContainer that generates a single custom secret"
        ...
        when: "The DeployCommand processes the BareBonesDeployable"
        new DeployCommand<>(deployable).execute(k8sClient, Optional.empty())

        then: "I expect to see the secret created by the BareBonesContainer"
        ...
        and: "the secret volume is mounted in the standard location"
        ...
        and: "the secret volume is mapped to the previously created secret"
    ...
    def "the serviceaccount is created with the necessary role based access to the specified resources in the namespace"() {
        ...
        and: "I use the BareBonesDeployable that requires a custom service account"
        def deployable = new BareBonesDeployable(customResource)
        emulatePodWaitBehavior(customResource, TEST_APP_NAME + "-" + BareBonesDeployable.NAME_QUALIFIER + DeploymentCreator.DEPLOYMENT_SUFFIX)
        ...
        and: "the a Role in the project that grants the permissions required by the BareBonesContainer"
        ...
        and: "the previously created role has been bound to the custom service account"
    ...
    def "the deployment is created reflecting the specs of the deployable implementation used"() {
        ...
        and: "I have a basic deployable implementation in the form of the BareBonesDeployable"
        def deployable = new BareBonesDeployable(customResource)
        emulatePodWaitBehavior(customResource, TEST_APP_NAME + "-" + BareBonesDeployable.NAME_QUALIFIER + DeploymentCreator.DEPLOYMENT_SUFFIX)
        ...
        and: "a container and port reflecting the name qualifier of the BarBonesContainer"
        ...
        and: "the resource limits specified by the BareBonesDeployableContainer "
        ...
        and: "the resource request that are a quarter of the resource limits "
